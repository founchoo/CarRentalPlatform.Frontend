  ...
  <MDBModal tag="form" id="addDataModal" tabindex="-1" v-model="common.showAddDataModal.value"
    ...
    <MDBModalBody>
      ...
      <div v-for="column in common.columns.value">
        <div v-if="column.type === 'boolean'" class="ms-1">
          <span>{{ column.label }}</span>
          <div v-if="column.field === 'gender'" class="d-flex flex-row mb-3">
            <MDBRadio :disabled="column.autoGenerated === true" :is-valid="column.isValid" isValidated label="男"
              value="true" v-model="common.data.value[column.field]" inline />
            <MDBRadio :disabled="column.autoGenerated === true" :is-valid="column.isValid" isValidated label="女"
              value="false" v-model="common.data.value[column.field]" inline />
          </div>
          <div v-else class="d-flex flex-row mb-3">
            <MDBRadio :disabled="column.autoGenerated === true" :is-valid="column.isValid" isValidated label="是"
              value="true" v-model="common.data.value[column.field]" inline />
            <MDBRadio :disabled="column.autoGenerated === true" :is-valid="column.isValid" isValidated label="否"
              value="false" v-model="common.data.value[column.field]" inline />
          </div>
        </div>
        <MDBInput v-else required :disabled="column.autoGenerated === true"
          :label="column.label + (column.autoGenerated ? '（自动生成）' : '')" v-model="common.data.value[column.field]"
          class="mb-md-4" :type="column.type" @change="column.validator(common.data.value[column.field])"
    ...
    <MDBModalFooter>
      ...
      <MDBBtn color="primary" type="submit" :disabled="common.columns.value.findIndex(item => {
